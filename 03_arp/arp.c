...
static void printmac(struct rte_ether_addr macaddr) {
...
static int ng_encode_arp_pkt(uint8_t *msg, uint8_t *dst_mac, uint32_t sip, uint32_t dip) {
	...
	rte_memcpy(arp->arp_data.arp_sha.addr_bytes, gSrcMac, RTE_ETHER_ADDR_LEN);  // 需要响应的本机mac
...
int main(int argc, char *argv[]) {
	...
	rte_eth_macaddr_get(gDpdkPortId, (struct rte_ether_addr *)gSrcMac); // 获取本机mac
	...
	while (1) {
		...
		for (i = 0;i < num_recvd;i++) {
			...
			if (ehdr->ether_type == rte_cpu_to_be_16(RTE_ETHER_TYPE_ARP)) {
				...
				if (ahdr->arp_data.arp_tip == gLocalIp) {  // 判断arp的目的
