...
static void ng_init_port(struct rte_mempool *mbuf_pool) {
	...
	if (rte_eth_rx_queue_setup(gDpdkPortId, 0, 128, rte_eth_dev_socket_id(gDpdkPortId), NULL, mbuf_pool) < 0) {  // 设置网卡的RX队列
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		for (i = 0;i < num_recvd;i++) {
			...
			if (ehdr->ether_type != rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4)) {  // rte_cpu_to_be_16从主机序转为网络序，判断是不是ipv4
				continue;
			}

			struct rte_ipv4_hdr *iphdr = rte_pktmbuf_mtod_offset(mbufs[i], struct rte_ipv4_hdr *, sizeof(struct rte_ether_hdr)); // 取出第i个数据包的ip头部
