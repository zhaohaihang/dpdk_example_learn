...
static uint32_t gLocalIp = MAKE_IPV4_ADDR(192, 168, 3, 110);
...
static void printmac(const struct rte_ether_addr *eth_addr) {
...
static void ng_init_port(struct rte_mempool *mbuf_pool) {
	uint16_t nb_sys_ports = rte_eth_dev_count_avail(); // 检查有没有可用的网口
	if (nb_sys_ports == 0) {
	...
	rte_eth_dev_configure(gDpdkPortId, num_rx_queues, num_tx_queues, &port_conf); // 配置网口

	if (rte_eth_rx_queue_setup(gDpdkPortId, 0, 1024, rte_eth_dev_socket_id(gDpdkPortId), NULL, mbuf_pool) < 0) { // 配置网口的RX队列
	...
	if (rte_eth_tx_queue_setup(gDpdkPortId, 0, 1024, rte_eth_dev_socket_id(gDpdkPortId), &txq_conf) < 0) { // 配置网口的TX队列
		rte_exit(EXIT_FAILURE, "Could not setup TX queue\n");
	}

	if (rte_eth_dev_start(gDpdkPortId) < 0) {  // 启动网口
...
static struct rte_mbuf *ng_send_arp(struct rte_mempool *mbuf_pool, uint16_t opcode, uint8_t *dst_mac, uint32_t sip, uint32_t dip) {
	...
	ng_encode_arp_pkt(pkt_data, opcode, dst_mac, sip, dip); //往mbuf中填充要发送的报文内容
...
static void  arp_request_timer_cb(__attribute__((unused)) struct rte_timer *tim, void *arg) {
	...
	struct inout_ring *ring = ringInstance(); // 通过单例模式获取ring

	int i = 0;
	for (i = 1;i <= 254;i++) {
		uint32_t dstip = (gLocalIp & 0x00FFFFFF) | (0xFF000000 & (i << 24)); // 遍历vlan内每一个ip
		struct rte_mbuf *arpbuf = NULL;
		uint8_t *dstmac = ng_get_dst_macaddr(dstip); // 根据ip查询arp中的mac地址
		if (dstmac == NULL) {  // 如果没有mac，以太网头部中就用全F
			arpbuf = ng_send_arp(mbuf_pool, RTE_ARP_OP_REQUEST, gDefaultArpMac, gLocalIp, dstip);
		}
		else {  // 如果查到了，以太网头部中就用查到的
			arpbuf = ng_send_arp(mbuf_pool, RTE_ARP_OP_REQUEST, dstmac, gLocalIp, dstip);
		}
		rte_ring_mp_enqueue_burst(ring->out, (void **)&arpbuf, 1, NULL);  // 将构建好的
	}
}

// 从ring-in取出包，处理后放到ring-out
static int pkt_process(void *arg) {
	...
	while (1) {
		...
		unsigned num_recvd = rte_ring_mc_dequeue_burst(ring->in, (void **)mbufs, BURST_SIZE, NULL); //从ring-in出队列，放在mbufs

		unsigned i = 0;
		for (i = 0;i < num_recvd;i++) {
			struct rte_ether_hdr *ehdr = rte_pktmbuf_mtod(mbufs[i], struct rte_ether_hdr *);
			printf("eth->srcmac:");printmac(&(ehdr->s_addr));printf("\n");
			printf("eth->dstmac:");printmac(&(ehdr->d_addr));printf("\n");
			// arp
			if (ehdr->ether_type == rte_cpu_to_be_16(RTE_ETHER_TYPE_ARP)) {
				struct rte_arp_hdr *ahdr = rte_pktmbuf_mtod_offset(mbufs[i], struct rte_arp_hdr *, sizeof(struct rte_ether_hdr));
				printf("    arp->srcmac:");printmac(&(ahdr->arp_data.arp_sha));printf("\n");
				printf("    arp->srcip:");printip(ahdr->arp_data.arp_sip);printf("\n");
				printf("    arp->dstmac:");printmac(&(ahdr->arp_data.arp_tha));printf("\n");
				printf("    arp->dstip:");printip(ahdr->arp_data.arp_tip);printf("\n");

				if (ahdr->arp_data.arp_tip == gLocalIp) { // 判断 arp的目的ip，是不是本机，即是不是发给本机的
					if (ahdr->arp_opcode == rte_cpu_to_be_16(RTE_ARP_OP_REQUEST)) { // 如果是arp请求报文，则回arp响应报文
						...
						rte_ring_mp_enqueue_burst(ring->out, (void **)&arpbuf, 1, NULL); // 将构建的arp 报文放入 ring-out
					}
					else if (ahdr->arp_opcode == rte_cpu_to_be_16(RTE_ARP_OP_REPLY)) { //arp 响应报文，则要更新arp表
						struct arp_table *table = arp_table_instance();
						uint8_t *hwaddr = ng_get_dst_macaddr(ahdr->arp_data.arp_sip); // 根据arp响应报文中的源mac，查询arp表
						if (hwaddr == NULL) {  // 如果没查到，则要新建arp表项
			...
			// ipv4
				...
				printf("    ipv4->srcip:");printip(iphdr->src_addr);printf("\n");
				printf("    ipv4->dstip:");printip(iphdr->dst_addr);printf("\n");
				// udp
				if (iphdr->next_proto_id == IPPROTO_UDP) {
					struct rte_udp_hdr *udphdr = (struct rte_udp_hdr *)(iphdr + 1);
					printf("        udp->srcport:%d\n", ntohs(udphdr->src_port));
					printf("        udp->dstport:%d\n", ntohs(udphdr->dst_port));
...
int main(int argc, char *argv[]) {
	...
	rte_timer_reset(&arp_timer, hz, PERIODICAL, lcore_id, arp_request_timer_cb, mbuf_pool);  // 通过定时器，持续的构建arp请求，并存到ring-out中
#endif
	...
	if (ring->in == NULL) {
		ring->in = rte_ring_create("in ring", RING_SIZE, rte_socket_id(), RING_F_SP_ENQ | RING_F_SC_DEQ);  // 创建ring-in
	}
	if (ring->out == NULL) {
		ring->out = rte_ring_create("out ring", RING_SIZE, rte_socket_id(), RING_F_SP_ENQ | RING_F_SC_DEQ); // 创建ring-out
	}
#endif

#if ENABLE_MULTHREAD
	rte_eal_remote_launch(pkt_process, mbuf_pool, rte_get_next_lcore(lcore_id, 1, 0)); // 启动一个线程，从ring-in中取出数据，处理后放到ring-out中
#endif

	while (1) {
		// rx 接收
		struct rte_mbuf *rx[BURST_SIZE];
		unsigned num_recvd = rte_eth_rx_burst(gDpdkPortId, 0, rx, BURST_SIZE); // 从rx队列中接收多个数据包，让入rx mbuf
		if (num_recvd > BURST_SIZE) {
			rte_exit(EXIT_FAILURE, "Error receiving from eth\n");
		}
		else if (num_recvd > 0) {
			rte_ring_sp_enqueue_burst(ring->in, (void **)rx, num_recvd, NULL); // 从rx mbuf中 存入ring-in，有多种入队列的函数， rx的内存在协议栈内释放（处理完才能释放）
		}

		// tx 发送
		struct rte_mbuf *tx[BURST_SIZE];
		unsigned nb_tx = rte_ring_sc_dequeue_burst(ring->out, (void **)tx, BURST_SIZE, NULL); // 从ring-out取出多个要发送的包，放入tx mbuf
		if (nb_tx > 0) {
			rte_eth_tx_burst(gDpdkPortId, 0, tx, nb_tx);  // 从tx mbuf中发送数据
			unsigned i = 0;
		...
		if (diff_tsc > TIMER_RESOLUTION_CYCLES) {
			rte_timer_manage();  // 间隔固定时间，调用发包函数
			prev_tsc = cur_tsc;
		}
#endif
