...
main(int argc, char **argv)
{
	...
	RTE_ETH_FOREACH_DEV(portid) {
		...
		qconf->rx_port_list[qconf->n_rx_port] = portid; // 记录locre负责的网卡id
		qconf->n_rx_port++;
	...
	RTE_ETH_FOREACH_DEV(portid) {
		...
		ret = rte_eth_dev_configure(portid, 1, 1, &local_port_conf); // 设置一个网口的tx rx队列数量 为1
		if (ret < 0)
			rte_exit(EXIT_FAILURE, "Cannot configure device: err=%d, port=%u\n",
				  ret, portid);

		ret = rte_eth_dev_adjust_nb_rx_tx_desc(portid, &nb_rxd,  // 设置一个网口的tx rx队列的描述符数量 为1024
						       &nb_txd);
		if (ret < 0)
		...
		rte_eth_macaddr_get(portid, &l2fwd_ports_eth_addr[portid]); // 获取网口mac

		/* init one RX queue */
		fflush(stdout);
		rxq_conf = dev_info.default_rxconf;
		rxq_conf.offloads = local_port_conf.rxmode.offloads;
		ret = rte_eth_rx_queue_setup(portid, 0, nb_rxd,    // 配置rx
		...
		fflush(stdout);
		ret = rte_eth_tx_queue_setup(portid, 0, nb_txd, // 配置tx
				rte_eth_dev_socket_id(portid),
				&txq_conf);
		if (ret < 0)
			rte_exit(EXIT_FAILURE,
		...
		tx_buffer[portid] = rte_zmalloc_socket("tx_buffer",   // 设置端口的发送缓冲区
